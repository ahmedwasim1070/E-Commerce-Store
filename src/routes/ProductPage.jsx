  ...
  return (
    <main className="bg-img w-full h-[100vh] overflow-x-hidden">
      <Helmet>
        ...
        <meta
          name="keywords"
          content={`Crush Fragrance, Crush Perfumes, Premium Fragrances, Perfumes for Men, Perfumes for Women, Best Fragrance Store, Buy Perfumes Online, Crush Fragrance Shop, Signature Scents, Exclusive Perfumes , ${productData[id].productName} Fragrance , For ${productData[id].productType} ,  ${productData[id].productName} Perfume ,  ${productData[id].productName} `}
        />
        <meta
          name="description"
          content="Love at Frist Scent > Check Out our Premium Fragrances Now"
        />
      </Helmet>
      ...
      <div className="w-full mx-auto fl:container text-white">
        ...
        {shSmNav ? (
          ...
          >
            <div
              onClick={smNavClick}
              className="w-[50%] h-full bg-white border border-solid rounded-lg slide-in"
            >
...
function Productrender({ id, viewForm, statusColor }) {
  ...
  let [isRatingRequired, setIsRatingRequired] = useState(false);
  let [avgRatings, setAvgRatings] = useState(0);
  ...
  const reviewAverage = () => {
    const totalStars = reviewData[id].reduce(
      (sum, review) => sum + (review.star || 0),
      0
    );
    const averageRatings = totalStars / reviewData[id].length;
    setAvgRatings(averageRatings.toFixed(1));
  };
  const chkRevData = () => {
    if (reviewData[id].length > 0) {
      setShowReview(true);
    }
  };
  ...
  return (
    <main className="w-full">
      <section className="w-full h-full flex pt-20 text-black  gap-x-10 gap-y-10 flex-wrap 2xl:justify-center xl:justify-center lg:justify-center md:justify-start sm:justify-start esm:justify-start">
        <div
          className={`w-[350px] h-[550px] p-5 2xl:mx-0 xl:mx-0 lg:mx-0 md:mx-auto sm:mx-auto esm:mx-auto ${productData[id].productBg}`}
        >
          <img
            className="mx-auto"
            loading="lazy"
            alt={`${productData[id].productName}`}
            src={`/productimg/${productData[id].productImg}`}
          />
        </div>
        <section className="2xl:w-[40%] xl:w-[40%] lg:w-[40%] md:w-[50%] sm:w-[100%] h-full ml-10 flex flex-col gap-y-4">
          <div className="flex gap-y-1 flex-col">
            ...
            <p className="text-2xl text-[rgba(0,0,0,0.6)]">
              {productData[id].productTagline}
            </p>
            <p
              className={`text-lg text-[rgba(0,0,0,0.6)] ${
                statusColor && "text-red-800 horizontal-shake"
              }`}
            >
              {productData[id].productStatus}
            </p>
            {showReview ? (
              <div className="flex  items-center">
                {[1, 2, 3, 4, 5].map((star) => (
                  <span
                    ...
                    style={{
                      cursor: "pointer",
                      color: star <= avgRatings ? "black" : "gray",
                      fontSize: " 30px ",
                    }}
                  >
                    ★
                ...
                <p className="ml-4 text-lg font-bold text-black/70">
                  ( {reviewData[id].length} )
                </p>
              </div>
            ) : (
          ...
          <div>
            <button
              onClick={viewForm}
              className={`w-[60%] border-2  p-3 rounded-sm text-lg transition-all duration-300 hover:bg-black my-4 hover:text-white outline-none horizontal-shake  ${
                statusColor
                  ? "border-red-800 horizontal-shake text-red-800"
                  : "border-[rgba(0,0,0,0.9)] text-black"
              }`}
            >
              Buy Now &gt;&gt;
            </button>
          ...
          <div className="flex flex-col gap-y-1 flex-wrap">
            ...
            <p className="text-lg font-bold text-[rgba(0,0,0,0.5)]">
              Inspired By {productData[id].productIns}
            </p>
            <div className="2xl:w-[70%] xl:w-[70%] lg:w-[70%] md:w-[80%] sm:w-[85%] esm:w-[95%]">
              <p className="text-wrap text-md text-[rgba(0,0,0,0.8)]">
                {productData[id].productAccords}
              </p>
            </div>
          </div>
        ...
        <section className="w-full">
          ...
          <div
            ...
            className={`w-[80%] my-2 text-lg overflow-hidden mx-auto  cursor-pointer  ${
              desDrop ? "max-h-[2000px]" : "h-8"
            } 2xl:px-20 xl:px-20 lg:px-14 md:px-2 sm:px-1 esm:px-0`}
          >
            <h1 className="w-full px-2 flex gap-4 items-center justify-between font-bold">
              Description
              {desDrop ? (
                <svg
                  width="20px"
            ...
            <div className="p-5 ">
              <h1 className="text-2xl font-semibold">
                {productData[id].productName} by Crush Fragrances :{" "}
              </h1>
              <div className="p-5">
                <p
                  dangerouslySetInnerHTML={{
                    __html: productData[id].productDes,
                  }}
                  className="text-sm"
                ></p>
              </div>
          ...
          <div
            onClick={() => {
              setNoteDrop(!noteDrop);
            }}
            className={`w-[80%] my-2 text-lg mx-auto px-20 overflow-hidden cursor-pointer ${
              noteDrop ? "max-h-[1000px]" : "h-8"
            }  2xl:px-20 xl:px-20 lg:px-14 md:px-2 sm:px-1 esm:px-0`}
          >
            <h1 className="w-full px-2 flex gap-4 items-center justify-between font-bold">
              Notes{" "}
            ...
            <div className="p-2">
              ...
              <div className="p-5 mt-5 w-[90%] h-full flex flex-col gap-y-3  ">
                <div className="flex flex-nowrap gap-8">
                  <h1 className="w-[45%] text-nowrap text-end font-bold text-[1rem]">
                    Top Notes :
                  </h1>
                  <p className="w-[55%] text-[rgba(0,0,0,0.5)] text-sm">
                    {productData[id].productTNotes}
                  </p>
                </div>
                <div className="flex  flex-nowrap gap-8">
                  <h1 className="w-[45%] text-nowrap text-end font-bold text-[1rem]">
          ...
          <div
            className={`w-[80%] my-2 text-lg mx-auto px-20 overflow-hidden cursor-pointer  ${
              revDropdown ? "max-h-[2000px]" : "h-8"
            }  2xl:px-20 xl:px-20 lg:px-14 md:px-2 sm:px-1 esm:px-0`}
          >
            <h1
              onClick={() => {
                setRevDropdown(!revDropdown);
              }}
              className="w-full px-2 flex gap-4 items-center justify-between font-bold"
            >
              Reviews ({reviewData[id].length})
              {revDropdown ? (
                <svg
                  width="20px"
                  height="20px"
            ...
            <div className="w-full flex justify-end my-5">
              <button
                onClick={() => setRevForm(!revForm)}
                className="bg-black p-3 text-white transition-all duration-300 rounded-md outline-none border border-[rgba(0,0,0,0.6)] hover:text-black hover:bg-white"
              >
                Write a Review
              </button>
            </div>
            ...
            {revForm && (
              <section className="w-full my-10 flex items-center justify-center">
                <form
                  ...
                  className="2xl:w-[50%] xl:w-[55%] lg:w-[60%] sm:w-[75%] esm:w-full p-5 rounded-xl flex gap-y-5 flex-col border border-solid shadow-2xl"
                >
                  ...
                  <div className="w-full flex flex-col gap-y-2">
                    <label
                      className={`w-full ${
                        errorState.name ? ` text-red-600` : ` text-black`
                      }`}
                    >
                      Name :{" "}
                    </label>
                    <input
                      ...
                      value={revFormData.name}
                      required
                      className={`w-full bg-transparent border  rounded-md p-2.5   outline-none  ${
                        errorState.name
                          ? "border-red-600 placeholder:text-red-600 "
                          : "border-[rgba(0,0,0,0.6)] placeholder:text-gray-450"
                      } `}
                  ...
                  <div className="w-full flex flex-col gap-y-2">
                    <label
                      className={`w-full ${
                        errorState.email ? ` text-red-600` : ` text-black`
                      }`}
                    >
                      Email :
                    </label>
                    <input
                      ...
                      type="text"
                      onChange={handleChange}
                      value={revFormData.email}
                      required
                      className={`w-full bg-transparent border  rounded-md p-2.5   outline-none border-[rgba(0,0,0,0.6)] placeholder:text-gray-450 ${
                        errorState.email
                          ? " border-red-600 placeholder:text-red-600 "
                          : " border-[rgba(0,0,0,0.6)] placeholder:text-gray-450 "
                      } `}
                    ...
                    <div className="w-full flex justify-center my-5">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <span
                          ...
                          style={{
                            cursor: "pointer",
                            color: star <= revFormData.rating ? "gold" : "gray",
                            fontSize: " 30px ",
                          }}
                        >
                          ★
                  ...
                  <div className="w-full flex flex-col gap-y-2">
                    <label
                      className={`w-full ${
                        errorState.message ? ` text-red-600` : ` text-black`
                      }`}
                    >
                      Message :
                    </label>
                    <textarea
                      ...
                      required
                      onChange={handleChange}
                      value={revFormData.message}
                      className={`w-full h-40 bg-transparent border rounded-md p-2.5 outline-none resize-none ${
                        errorState.message
                          ? "border-red-600 placeholder:text-red-600 "
                          : "border-[rgba(0,0,0,0.6)] placeholder:text-gray-450"
                      } `}
                    ...
                    {errorState.message && (
                      <p className="text-red-600">
                        Message should be under 256 characters
                      </p>
                    )}
                  </div>
                  <div className="w-full flex justify-center">
                    <button
                      ...
                      className="flex items-center justify-center border w-full  border-black  my-5 p-4 text-xl bg-black text-white rounded-2xl transition-all duration-300 hover:bg-white hover:text-black"
                    >
                      Post &gt; &gt;
                    </button>
                  </div>
            ...
            <div className="w-full px-10 grid grid-cols-1 gap-3">
              {showReview ? (
                reviewData[id].map((id) => {
                  return (
                    <>
                      <hr />
                      <div
                        key={id}
                        className="flex flex-col gap-2 my-2 w-[90%]"
                      >
                        <div>
                          {[1, 2, 3, 4, 5].map((star) => (
                            <span
                        ...
                        <p className=" text-black/70">{id.message}</p>
                      </div>
                      <hr />
                    </>
                  );
...
function Userform({ formRef, id }) {
  ...
  const [productTotal, setProductTotal] = useState(
    Number(productData[id].productPrice) - Number(productData[id].productSale)
  );
  ...
  const [formData, setFormData] = useState({
    ...
    productData: productData[id].productName,
    productTotal:
      productData[id].productPrice - productData[id].productSale * 1,
  });
  ...
  const handleProvinceChange = (e) => {
    ...
    setFormData({
      ...
      city: countryCity[newProvince][0]?.value,
    });
  };
  ...
  const setVal = (e) => {
    if (e.target.name === "neg") {
      if (inputValue > 1) {
        ...
        setProductTotal(
          productTotal -
            Number(productData[id].productPrice - productData[id].productSale)
        );
  ...
  return (
    <section
      ref={formRef}
      className="w-full 2xl:p-10 xl:p-10 lg:p-10 md:p-10 sm:p-0 esm:p-0 relative"
    >
      <form
        onSubmit={handleSubmit}
        className="border border-solid shadow-2xl 2xl:p-10 xl:p-10 lg:p-10 md:p-8 sm:p-6 esm:p-4 my-10 rounded-2xl flex flex-col gap-10 text-black"
      >
        <ToastContainer />
        <h1 className="text-center 2xl:text-[3rem] xl:text-[3rem] lg:text-[2.5rem] md:text-[2.5rem] sm:text-[2rem] esm:text-[2rem]">
          Enter Your Details
        </h1>
        ...
        <div className="flex items-center gap-x-5 flex-wrap mt-5">
          <input
            ...
            type="text"
            required
            className={` 2xl:w-[30%] xl:w-[30%] lg:w-[30%] md:w-[80%] sm:w-[80%] esm:w-full bg-transparent border  rounded-md p-2.5   outline-none ${
              errorState.name
          ...
          />
          {errorState.name && (
            <p className="w-full mt-2 text-red-600 text-sm">
              Enter a valid Name (Must be smaller then 24 characters)
            </p>
          )}
        </div>
        <div className="flex items-center gap-x-5 flex-wrap">
          <input
            ...
            type="tel"
            required
            className={`2xl:w-[30%] xl:w-[30%]  lg:w-[85%] md:w-[85%] sm:w-[85%] esm:w-full bg-transparent border border-[rgba(0,0,0,0.6)] rounded-md p-2.5 placeholder:text-gray-450 outline-none ${
              errorState.phone
                ? "border-red-600  placeholder:text-red-600  text-red-600 "
                : "border-[rgba(0,0,0,0.6)] placeholder:text-gray-450 "
            } `}
          ...
          {errorState.phone && (
            <p className=" w-full mt-2 text-red-600 text-sm">
              Enter a valid Phone Number (Number Should be 11 digits)
            </p>
          )}
        </div>
        <div className="flex items-center gap-x-5 flex-wrap">
          <input
            ...
            type="email"
            required
            className={`2xl:w-[40%] xl:w-[40%] lg:w-[65%] sm:w-[75%] esm:w-full bg-transparent border border-[rgba(0,0,0,0.6)] rounded-md p-2.5 placeholder:text-gray-450 outline-none ${
              errorState.email
                ? "border-red-600  placeholder:text-red-600 text-red-600 "
                : "border-[rgba(0,0,0,0.6)] placeholder:text-gray-450 "
            }`}
          ...
          {errorState.email && (
            <p className="w-full mt-2 text-red-600 text-sm">
              Enter a valid Email (Email should be smaller then 48 characters)
            </p>
          )}
        </div>
        <div className="flex items-center gap-5 flex-wrap">
          ...
          <select
            ...
            name="province"
            required
            className=" 2xl:w-[30%] xl:w-[30%] lg:w-[30%] md:w-[80%] sm:w-[80%] esm:w-full  border border-[rgba(0,0,0,0.6)] rounded-md p-2.5  outline-none cursor-pointer bg-white text-gray-500 "
          >
        ...
        <div className="flex items-center gap-5 flex-wrap">
          <label className="text-lg text-[rgba(0,0,0,0.7)]">
            Cities of{" "}
            {formData.province.charAt(0).toUpperCase() +
              formData.province.slice(1)}{" "}
            :
          </label>
          <select
            value={formData.city}
        ...
        <div className="flex items-center gap-5 flex-wrap">
          ...
          <div className="flex items-center overflow-hidden">
            <button
              ...
              onClick={setVal}
              type="button"
              className="px-3 rounded-sm bg-black text-white text-lg  "
            >
              -
            </button>
            <input
              ...
              value={inputValue}
              min="1"
              className="w-[40px] bg-transparent border border-solid outline-none px-3 border-[rgba(0,0,0,0.6)] text-gray-500 rounded-sm  cursor-default"
              required
              readOnly
            />
            <button
        ...
        <div className="flex items-center gap-x-5 flex-wrap">
          <textarea
            ...
            name="address"
            required
            className={`2xl:w-[45%] xl:w-[45%] lg:w-[45%] md:w-[80%] sm:w-[80%] esm:w-full h-40 bg-transparent border border-[rgba(0,0,0,0.6)] rounded-md p-2.5 placeholder:text-gray-500 outline-none resize-none ${
              errorState.address
                ? "border-red-600  placeholder:text-red-600 text-red-600  "
                : "border-[rgba(0,0,0,0.6)] placeholder:text-gray-450 "
            }`}
          ...
          {errorState.address && (
            <p className=" w-full mt-2 text-red-600 text-sm">
              Enter a valid Address (Should be greater then 24 and smaller then
              256 Characters){" "}
            </p>
          )}
        </div>
        <button
          type="button"
          className="py-5 px-3 2xl:w-[50%] xl:w-[50%] lg:w-[50%] md:w-full sm:w-full esm:w-full border border-black rounded-lg cursor-default outline-none"
        >
          <div className="flex items-center justify-center gap-x-3 flex-nowrap">
            ...
            <svg viewBox="0 0 24 24" fill="#00000" height="24px" width="24px">
              {" "}
        ...
        <div className="">
          ...
          <div className="my-5">
            <img
              className="w-[125px] shadow-2xl p-2 border border-solid rounded-md"
              loading="lazy"
              alt={`${productData[id].productName}`}
              src={`/productimg/${productData[id].productImg}`}
            />
          ...
          <div className="flex gap-x-2 items-center">
            ...
            <p className="text-[1.2rem] text-gray-600 flex gap-x-2">
              {" "}
              {productTotal} + <span className="line-through">200 </span> ={" "}
              {productTotal} {/*{inputValue*200}*/}{" "}
            </p>
          </div>
          <p className="text-red-600 text-sm mt-5">
            After placing Order you will be expecting a Confirmation Phone Call
            . Must be Answered In-order to Make your Purchase.{" "}
          </p>
        </div>
        <div className="w-full h-[100px] flex flex-col gap-y-3 items-center justify-center">
          <button
            ...
            type="submit"
            className="p-3 px-5 border border-[rgba(0,0,0,0.6)] rounded-2xl text-[1.5rem] bg-black flex items-center gap-x-2 text-white transition-all duration-300 hover:bg-transparent hover:text-black group"
          >
            Place Order
            <svg
              width="32px"
