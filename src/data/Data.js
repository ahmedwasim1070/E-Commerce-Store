export let productData=[
    {
        'productId': 1,
        'productName': 'Shelby',
        'productPrice': '2300',
        'productSale': '301',
        'productImg': 'perfume-1.webp',
        'productTagline': 'Elegance meets boldness.',
        'productSize' : '50ml',
        'productType': 'For Men ',
        'productStatus': 'In-Stock',
        'productDes': `Unleash your inner confidence with Shelby, a fragrance inspired by the iconic Sauvage by Dior, embodying the spirit of freedom and rugged masculinity. From the very first spritz, the vibrant Calabrian bergamot and a hint of pepper awaken the senses, introducing a fresh and invigorating top note that sets the stage for what’s to come.<br/><br/>As the scent unfolds, a harmonious blend of Sichuan pepper, lavender, and pink pepper dances at the heart, intertwined with earthy nuances of vetiver, patchouli, geranium, and elemi. This sophisticated mix exudes a bold yet refined character, perfect for the modern man who embraces every moment with confidence.<br/><br/>The journey concludes with the rich depth of ambroxan, cedar, and labdanum, which linger on the skin, leaving a trail of intrigue. Designed for both young and mature men, Shelby is versatile enough to transition seamlessly from day to night.<br/><br/>Ideal for casual outings, professional settings, or romantic evenings, Shelby is the perfect companion for spring, summer, and fall. Its fresh, spicy aroma captivates, while its outstanding longevity ensures you make a lasting impression.<br/><br/>Awaken your senses and express your individuality with Shelby, a scent that redefines masculinity and sophistication.<br/>`,
        'productBg' : 'bg-perfume-1',
        'productNotesimg': 'perfume-1-notes.webp',
        'productIns': 'Sauvage Dior ',
        'productAccords': 'Fresh Spicy, Amber, Citrus, Aromatic, Musky, Woody, Herbal, Lavendar, Warm Spicy',
        'productTNotes': 'Calabrian bergamot , Pepper.',
        'productMNotes': 'Sichuan Pepper , Lavender , Pink Pepper , Vetiver , Patchouli , Geranium , Elemi',
        'productBNotes': 'Ambroxan , Cedar , Labdanum'
    },
    {
        'productId': 2,
        'productName': 'Divine',
        'productPrice': '2300',
        'productSale': '301',
        'productImg': 'perfume-2.webp',
        'productTagline': 'A scent touched by heaven.',
        'productSize' : '50ml',
        'productType': 'For Men / Women',
        'productStatus': 'In-Stock',
        'productDes': `Step into a world of opulence and sophistication with Divine, a luxurious unisex fragrance that embodies elegance and boldness in every spritz. Inspired by the exquisite allure of Baccarat 540, this captivating scent invites you to experience a warm embrace of amber and the ethereal beauty of jasmine, intertwining with the grounding notes of cedarwood.<br/><br/>Divine unfolds with a fresh spiciness that dances alongside woody undertones, evoking a sense of mystery and confidence. As the fragrance develops, its animalic and metallic nuances create a signature aura that lingers in the air, leaving an unforgettable impression. Perfectly balanced, it resonates with the youthful spirit, making it ideal for both intimate gatherings and grand occasions.<br/><br/>With remarkable longevity and sillage, Divine is not just a fragrance; it’s an expression of elegance that transcends the ordinary. Unveil your allure with Divine—A scent touched by heaven, made for you<br/><br/>`,
        'productBg': 'bg-perfume-2',
        'productNotesimg': 'perfume-2-notes.webp',
        'productIns': 'Bakarat 540 ',
        'productAccords': 'Woody , Amber , Warm Spicy , Fresh Spicy , Metalic , Aromatic , White floral , Animalic',
        'productTNotes': 'Saffron , Jasmine ',
        'productMNotes': 'Amberwood , Ambergris , Hedione',
        'productBNotes': 'Fir Resin , Cedar , Sugar , Ambroxan , Oakmoss'
    },
    {
        'productId': 3,
        'productName': 'Emerald Mist',
        'productPrice': '2200',
        'productSale': '401',
        'productImg': 'perfume-3.webp',
        'productTagline': 'A Fresh Splash of Boldness',
        'productType': 'For Men / Women ',
        'productSize' : '50ml',
        'productStatus': 'In-Stock',
        'productDes': 'Dive into the depths of sophistication with Emerald Mist, a fragrance inspired by the iconic Acqua di Giò Profondo. Crafted for the man who is drawn to the untamed beauty of the ocean, this scent captures the essence of freshness and refined elegance.<br/><br/>The fragrance opens with invigorating top notes of sea notes, Aquozone, bergamot, and green mandarin, evoking a crisp sea breeze on a sunlit day. As it develops, the middle notes of rosemary, lavender, cypress, and mastic add a herbal touch, enveloping the wearer in a sophisticated yet natural aura that evokes the coastal Mediterranean.<br/><br/>At its base, mineral notes, musk, patchouli, and amber provide an earthy depth, adding boldness to the fresh aquatic character. Emerald Mist is best worn during spring and summer, perfect for daytime activities, casual outings, and outdoor adventures.<br/><br/>With its marine and citrus notes, Emerald Mist offers a refreshing and invigorating scent that boasts impressive longevity. This unique blend of modern elegance and bold, aquatic depth ensures that you stand out while feeling revitalized throughout the day.<br/><br/>Experience the freshness of the ocean with Emerald Mist—a scent that embodies the spirit of adventure and sophistication.<br/><br/>',
        'productBg': 'bg-perfume-3',
        'productNotesimg': 'perfume-3-notes.webp',
        'productIns': 'Acqua di Giò Profondo',
        'productAccords': 'Marine , Aromatic , Citrus , Fresh Spicy , Fresh , Woody , Mineral , Salty , Lavendar ',
        'productTNotes': 'Sea Notes , Aquozone , Bergamot , Green Mandarin ',
        'productMNotes': 'Rosemary , Lavender , Cypress , Mastic or Lentisque',
        'productBNotes': 'Mineral Notes , Musk , Patchouli , Amber',
    },
    {
        'productId': 4,
        'productName': 'Sky Veil',
        'productPrice': '2200',
        'productSale': '401',
        'productImg': 'perfume-4.webp',
        'productTagline': 'Make Your Mark with Sky Veil.',
        'productSize' : '50ml',
        'productType': 'For Men',
        'productStatus': 'In-Stock',
        'productDes': `Elevate your presence with Sky Veil, a captivating fragrance inspired by the luxurious Creed Aventus. This bold scent is crafted for the modern man who exudes confidence and charisma, making a statement wherever he goes. <br/> <br/> The journey begins with invigorating top notes of bergamot, black currant, apple, lemon, and a touch of pink pepper, creating a vibrant and refreshing opening that awakens the senses. As the fragrance evolves, the middle notes of pineapple, patchouli, and Moroccan jasmine intertwine, adding a lush and exotic twist that enhances its sophisticated allure. <br/> <br/> As it dries down, the fragrance reveals its powerful base notes of birch, musk, oak moss, ambroxan, and cedarwood, leaving a trail of smoky warmth and depth that lingers long after you’ve left the room. Sky Veil is ideal for formal events, special occasions, and evenings out, perfectly suited for spring and fall. <br/> <br/> With its exceptional longevity and projection, this fragrance embodies luxury and success, making it perfect for the discerning man who knows how to make an impact. Sky Veil's unique blend also offers a subtle unisex appeal, allowing versatility for various occasions.<br/> `,
        'productBg': 'bg-perfume-4',
        'productNotesimg': 'perfume-4-notes.webp',
        'productIns': 'Aventus Creed',
        'productAccords': 'Fruity , Sweet , Leather , Woody , Smoky , Citrus , Musky , Tropical , Fresh , Mossy',
        'productTNotes': 'Bergamot , Black Currant , Apple , Lemon , Pink Pepper ',
        'productMNotes': 'Pineapple , Patchouli , Moroccan Jasmine',
        'productBNotes': 'Birch , Musk , Oak Moss , Ambroxan , Cedar Wood',
    },
    {
        'productId': 5,
        'productName': 'Pink Whisper',
        'productPrice': '2000',
        'productSale': '301',
        'productImg': 'perfume-5.webp',
        'productTagline': 'Whispers of Charm and Confidence.',
        'productSize' : '50ml',
        'productType': 'For Women',
        'productStatus': 'In-Stock',
        'productDes': `Embrace the essence of femininity with Pink Whisper, a delightful fragrance inspired by the elegant Gucci Flora. This enchanting scent is designed for the modern woman who radiates cheerfulness and sophistication, making her presence felt in every room.<br/><br/>Embrace the essence of femininity with Pink Whisper, a delightful fragrance inspired by the elegant Gucci Flora. This enchanting scent is designed for the modern woman who radiates cheerfulness and sophistication, making her presence felt in every room.<br/><br/>The fragrance gently settles into its base notes of patchouli, musk, cedar, and leather, adding depth and a touch of sensuality, making it a perfect complement to its floral heart. Pink Whisper is ideally suited for spring and summer, perfect for daytime events, casual outings, or delightful brunches with friends.<br/><br/>With its light and refreshing scent, Pink Whisper offers moderate longevity and projection, ensuring you feel fresh and elegant throughout the day. This playful yet sophisticated fragrance captures the spirit of femininity, allowing you to express your unique style effortlessly.<br/><br/>Step into a world of elegance and freshness with Pink Whisper—a fragrance that whispers charm and confidence with every spritz.<br/>`,
        'productBg': 'bg-perfume-5',
        'productNotesimg': 'perfume-5-notes.webp',
        'productIns': 'Gucci Flora',
        'productAccords': 'Floral , Citrus , Fresh , Rose , Fruity , Woody , Soft Spicy , Patchouli , Warm Spicy',
        'productTNotes': 'Peony , Citruses , Mandarin Orange ',
        'productMNotes': 'Osmanthus  , Rose ',
        'productBNotes': 'Sandalwood , Pink Pepper , Patchouli',
    },
    {
        'productId': 6,
        'productName': 'Velvet Bloom',
        'productPrice': '2000',
        'productSale': '301',
        'productImg': 'perfume-6.webp',
        'productTagline': 'Let your radiance bloom.',
        'productSize' : '50ml',
        'productType': 'For Women',
        'productStatus': 'In-Stock',
        'productDes': `Step into a world of playful glamour with Velvet Bloom, a fragrance inspired by the iconic Victoria's Secret Bombshell. Designed for the woman who exudes confidence and a flirtatious charm, Velvet Bloom captures the essence of femininity with a bright, fresh twist.<br/><br/>The fragrance opens with top notes of passionfruit, grapefruit, pineapple, tangerine, and big strawberry, creating an irresistible burst of fruity sweetness that instantly energizes and captivates. The heart reveals a lush bouquet of peony, vanilla orchid, red berries, jasmine, and lily-of-the-valley, adding a floral elegance that is both inviting and glamorous.<br/><br/>As the scent settles, base notes of musk, woody notes, and oakmoss provide a soft, grounding finish, enhancing its flirtatious and sophisticated appeal. Velvet Bloom is ideal for spring and summer, perfect for daytime outings, casual settings, or fun social events.<br/><br/>With its enchanting blend of fruity and floral notes, Velvet Bloom offers a bright, youthful, and energetic character with moderate longevity and projection. It is the perfect companion for the woman who loves to shine wherever she goes.<br/><br/>`,
        'productBg': 'bg-perfume-6',
        'productNotesimg': 'perfume-5-notes.webp',
        'productIns': 'Bombshell',
        'productAccords': 'Fruity , Sweet , Fresh , Citrus , Floral , Tropical , White Floarl',
        'productTNotes': 'Passion Fruit , Grape Fruit , Pineapple , Tangerine , Big Strawberry',
        'productMNotes': 'Peony , Vanilla Orchid , Red Berries , Jasmine , Lily-of-the-Valley',
        'productBNotes': 'Musk , Woody Notes , Oak Moss',
    },
]



// City's of Pakistan (Azad Kashmir not included)


export let countryCity={
    punjab:[
        { value: "Islamabad", name: "Islamabad" },
        { value: "Ahmed Nager Chatha", name: "Ahmed Nager Chatha" },
        { value: "Ahmadpur East", name: "Ahmadpur East" },
        { value: "Ali Khan Abad", name: "Ali Khan Abad" },
        { value: "Alipur", name: "Alipur" },
        { value: "Arifwala", name: "Arifwala" },
        { value: "Attock", name: "Attock" },
        { value: "Bhera", name: "Bhera" },
        { value: "Bhalwal", name: "Bhalwal" },
        { value: "Bahawalnagar", name: "Bahawalnagar" },
        { value: "Bahawalpur", name: "Bahawalpur" },
        { value: "Bhakkar", name: "Bhakkar" },
        { value: "Burewala", name: "Burewala" },
        { value: "Chillianwala", name: "Chillianwala" },
        { value: "Chakwal", name: "Chakwal" },
        { value: "Chichawatni", name: "Chichawatni" },
        { value: "Chiniot", name: "Chiniot" },
        { value: "Chishtian", name: "Chishtian" },
        { value: "Daska", name: "Daska" },
        { value: "Darya Khan", name: "Darya Khan" },
        { value: "Dera Ghazi Khan", name: "Dera Ghazi Khan" },
        { value: "Dhaular", name: "Dhaular" },
        { value: "Dina", name: "Dina" },
        { value: "Dinga", name: "Dinga" },
        { value: "Dipalpur", name: "Dipalpur" },
        { value: "Faisalabad", name: "Faisalabad" },
        { value: "Ferozewala", name: "Ferozewala" },
        { value: "Fateh Jang", name: "Fateh Jang" },
        { value: "Ghakhar Mandi", name: "Ghakhar Mandi" },
        { value: "Gojra", name: "Gojra" },
        { value: "Gujranwala", name: "Gujranwala" },
        { value: "Gujrat", name: "Gujrat" },
        { value: "Gujar Khan", name: "Gujar Khan" },
        { value: "Hafizabad", name: "Hafizabad" },
        { value: "Haroonabad", name: "Haroonabad" },
        { value: "Hasilpur", name: "Hasilpur" },
        { value: "Haveli Lakha", name: "Haveli Lakha" },
        { value: "Jatoi", name: "Jatoi" },
        { value: "Jalalpur", name: "Jalalpur" },
        { value: "Jattan", name: "Jattan" },
        { value: "Jampur", name: "Jampur" },
        { value: "Jaranwala", name: "Jaranwala" },
        { value: "Jhang", name: "Jhang" },
        { value: "Jhelum", name: "Jhelum" },
        { value: "Kalabagh", name: "Kalabagh" },
        { value: "Karor Lal Esan", name: "Karor Lal Esan" },
        { value: "Kasur", name: "Kasur" },
        { value: "Kamalia", name: "Kamalia" },
        { value: "Kamoke", name: "Kamoke" },
        { value: "Khanewal", name: "Khanewal" },
        { value: "Khanpur", name: "Khanpur" },
        { value: "Kharian", name: "Kharian" },
        { value: "Khushab", name: "Khushab" },
        { value: "Kot Addu", name: "Kot Addu" },
        { value: "Jauharabad", name: "Jauharabad" },
        { value: "Lahore", name: "Lahore" },
        { value: "Lalamusa", name: "Lalamusa" },
        { value: "Layyah", name: "Layyah" },
        { value: "Liaquat Pur", name: "Liaquat Pur" },
        { value: "Lodhran", name: "Lodhran" },
        { value: "Malakwal", name: "Malakwal" },
        { value: "Mamoori", name: "Mamoori" },
        { value: "Mailsi", name: "Mailsi" },
        { value: "Mandi Bahauddin", name: "Mandi Bahauddin" },
        { value: "Mian Channu", name: "Mian Channu" },
        { value: "Mianwali", name: "Mianwali" },
        { value: "Multan", name: "Multan" },
        { value: "Murree", name: "Murree" },
        { value: "Muridke", name: "Muridke" },
        { value: "Mianwali Bangla", name: "Mianwali Bangla" },
        { value: "Muzaffargarh", name: "Muzaffargarh" },
        { value: "Narowal", name: "Narowal" },
        { value: "Nankana Sahib", name: "Nankana Sahib" },
        { value: "Okara", name: "Okara" },
        { value: "Renala Khurd", name: "Renala Khurd" },
        { value: "Pakpattan", name: "Pakpattan" },
        { value: "Pattoki", name: "Pattoki" },
        { value: "Pir Mahal", name: "Pir Mahal" },
        { value: "Qaimpur", name: "Qaimpur" },
        { value: "Qila Didar Singh", name: "Qila Didar Singh" },
        { value: "Rabwah", name: "Rabwah" },
        { value: "Raiwind", name: "Raiwind" },
        { value: "Rajanpur", name: "Rajanpur" },
        { value: "Rahim Yar Khan", name: "Rahim Yar Khan" },
        { value: "Rawalpindi", name: "Rawalpindi" },
        { value: "Sadiqabad", name: "Sadiqabad" },
        { value: "Safdarabad", name: "Safdarabad" },
        { value: "Sahiwal", name: "Sahiwal" },
        { value: "Sangla Hill", name: "Sangla Hill" },
        { value: "Sarai Alamgir", name: "Sarai Alamgir" },
        { value: "Sargodha", name: "Sargodha" },
        { value: "Shakargarh", name: "Shakargarh" },
        { value: "Sheikhupura", name: "Sheikhupura" },
        { value: "Sialkot", name: "Sialkot" },
        { value: "Sohawa", name: "Sohawa" },
        { value: "Soianwala", name: "Soianwala" },
        { value: "Siranwali", name: "Siranwali" },
        { value: "Talagang", name: "Talagang" },
        { value: "Taxila", name: "Taxila" },
        { value: "Toba Tek Singh", name: "Toba Tek Singh" },
        { value: "Vehari", name: "Vehari" },
        { value: "Wah Cantonment", name: "Wah Cantonment" },
        { value: "Wazirabad", name: "Wazirabad" }
    ],
    sindh:[
        { value: 'Badin', label: 'Badin' },
        { value: 'Bhirkan', label: 'Bhirkan' },
        { value: 'Rajo Khanani', label: 'Rajo Khanani' },
        { value: 'Chak', label: 'Chak' },
        { value: 'Dadu', label: 'Dadu' },
        { value: 'Digri', label: 'Digri' },
        { value: 'Diplo', label: 'Diplo' },
        { value: 'Dokri', label: 'Dokri' },
        { value: 'Ghotki', label: 'Ghotki' },
        { value: 'Haala', label: 'Haala' },
        { value: 'Hyderabad', label: 'Hyderabad' },
        { value: 'Islamkot', label: 'Islamkot' },
        { value: 'Jacobabad', label: 'Jacobabad' },
        { value: 'Jamshoro', label: 'Jamshoro' },
        { value: 'Jungshahi', label: 'Jungshahi' },
        { value: 'Kandhkot', label: 'Kandhkot' },
        { value: 'Kandiaro', label: 'Kandiaro' },
        { value: 'Karachi', label: 'Karachi' },
        { value: 'Kashmore', label: 'Kashmore' },
        { value: 'Keti Bandar', label: 'Keti Bandar' },
        { value: 'Khairpur', label: 'Khairpur' },
        { value: 'Kotri', label: 'Kotri' },
        { value: 'Larkana', label: 'Larkana' },
        { value: 'Matiari', label: 'Matiari' },
        { value: 'Mehar', label: 'Mehar' },
        { value: 'Mirpur Khas', label: 'Mirpur Khas' },
        { value: 'Mithani', label: 'Mithani' },
        { value: 'Mithi', label: 'Mithi' },
        { value: 'Mehrabpur', label: 'Mehrabpur' },
        { value: 'Moro', label: 'Moro' },
        { value: 'Nagarparkar', label: 'Nagarparkar' },
        { value: 'Naudero', label: 'Naudero' },
        { value: 'Naushahro Feroze', label: 'Naushahro Feroze' },
        { value: 'Naushara', label: 'Naushara' },
        { value: 'Nawabshah', label: 'Nawabshah' },
        { value: 'Nazimabad', label: 'Nazimabad' },
        { value: 'Qambar', label: 'Qambar' },
        { value: 'Qasimabad', label: 'Qasimabad' },
        { value: 'Ranipur', label: 'Ranipur' },
        { value: 'Ratodero', label: 'Ratodero' },
        { value: 'Rohri', label: 'Rohri' },
        { value: 'Sakrand', label: 'Sakrand' },
        { value: 'Sanghar', label: 'Sanghar' },
        { value: 'Shahbandar', label: 'Shahbandar' },
        { value: 'Shahdadkot', label: 'Shahdadkot' },
        { value: 'Shahdadpur', label: 'Shahdadpur' },
        { value: 'Shahpur Chakar', label: 'Shahpur Chakar' },
        { value: 'Shikarpaur', label: 'Shikarpaur' },
        { value: 'Sukkur', label: 'Sukkur' },
        { value: 'Tangwani', label: 'Tangwani' },
        { value: 'Tando Adam Khan', label: 'Tando Adam Khan' },
        { value: 'Tando Allahyar', label: 'Tando Allahyar' },
        { value: 'Tando Muhammad Khan', label: 'Tando Muhammad Khan' },
        { value: 'Thatta', label: 'Thatta' },
        { value: 'Umerkot', label: 'Umerkot' },
        { value: 'Warah', label: 'Warah' },
    ],
    kpk:[
        { value: 'Adezai', label: 'Adezai' },
        { value: 'Alpuri', label: 'Alpuri' },
        { value: 'Akora Khattak', label: 'Akora Khattak' },
        { value: 'Ayubia', label: 'Ayubia' },
        { value: 'Banda Daud Shah', label: 'Banda Daud Shah' },
        { value: 'Bannu', label: 'Bannu' },
        { value: 'Batkhela', label: 'Batkhela' },
        { value: 'Battagram', label: 'Battagram' },
        { value: 'Birote', label: 'Birote' },
        { value: 'Chakdara', label: 'Chakdara' },
        { value: 'Charsadda', label: 'Charsadda' },
        { value: 'Chitral', label: 'Chitral' },
        { value: 'Daggar', label: 'Daggar' },
        { value: 'Dargai', label: 'Dargai' },
        { value: 'Darya Khan', label: 'Darya Khan' },
        { value: 'Dera Ismail Khan', label: 'Dera Ismail Khan' },
        { value: 'Doaba', label: 'Doaba' },
        { value: 'Dir', label: 'Dir' },
        { value: 'Drosh', label: 'Drosh' },
        { value: 'Hangu', label: 'Hangu' },
        { value: 'Haripur', label: 'Haripur' },
        { value: 'Karak', label: 'Karak' },
        { value: 'Kohat', label: 'Kohat' },
        { value: 'Kulachi', label: 'Kulachi' },
        { value: 'Lakki Marwat', label: 'Lakki Marwat' },
        { value: 'Latamber', label: 'Latamber' },
        { value: 'Madyan', label: 'Madyan' },
        { value: 'Mansehra', label: 'Mansehra' },
        { value: 'Mardan', label: 'Mardan' },
        { value: 'Mastuj', label: 'Mastuj' },
        { value: 'Mingora', label: 'Mingora' },
        { value: 'Nowshera', label: 'Nowshera' },
        { value: 'Paharpur', label: 'Paharpur' },
        { value: 'Pabbi', label: 'Pabbi' },
        { value: 'Peshawar', label: 'Peshawar' },
        { value: 'Saidu Sharif', label: 'Saidu Sharif' },
        { value: 'Shorkot', label: 'Shorkot' },
        { value: 'Shewa Adda', label: 'Shewa Adda' },
        { value: 'Swabi', label: 'Swabi' },
        { value: 'Swat', label: 'Swat' },
        { value: 'Tangi', label: 'Tangi' },
        { value: 'Tank', label: 'Tank' },
        { value: 'Thall', label: 'Thall' },
        { value: 'Timergara', label: 'Timergara' },
        { value: 'Tordher', label: 'Tordher' },
    ],
    balochistan:[
        { value: 'Awaran', label: 'Awaran' },
        { value: 'Barkhan', label: 'Barkhan' },
        { value: 'Chagai', label: 'Chagai' },
        { value: 'Dera Bugti', label: 'Dera Bugti' },
        { value: 'Gwadar', label: 'Gwadar' },
        { value: 'Harnai', label: 'Harnai' },
        { value: 'Jafarabad', label: 'Jafarabad' },
        { value: 'Jhal Magsi', label: 'Jhal Magsi' },
        { value: 'Kacchi', label: 'Kacchi' },
        { value: 'Kalat', label: 'Kalat' },
        { value: 'Kech', label: 'Kech' },
        { value: 'Kharan', label: 'Kharan' },
        { value: 'Khuzdar', label: 'Khuzdar' },
        { value: 'Killa Abdullah', label: 'Killa Abdullah' },
        { value: 'Killa Saifullah', label: 'Killa Saifullah' },
        { value: 'Kohlu', label: 'Kohlu' },
        { value: 'Lasbela', label: 'Lasbela' },
        { value: 'Lehri', label: 'Lehri' },
        { value: 'Loralai', label: 'Loralai' },
        { value: 'Mastung', label: 'Mastung' },
        { value: 'Musakhel', label: 'Musakhel' },
        { value: 'Nasirabad', label: 'Nasirabad' },
        { value: 'Nushki', label: 'Nushki' },
        { value: 'Panjgur', label: 'Panjgur' },
        { value: 'Pishin Valley', label: 'Pishin Valley' },
        { value: 'Quetta', label: 'Quetta' },
        { value: 'Sherani', label: 'Sherani' },
        { value: 'Sibi', label: 'Sibi' },
        { value: 'Sohbatpur', label: 'Sohbatpur' },
        { value: 'Washuk', label: 'Washuk' },
        { value: 'Zhob', label: 'Zhob' },
        { value: 'Ziarat', label: 'Ziarat' },
    ],
}